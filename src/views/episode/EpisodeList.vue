<template>
  <div>
    <!-- 列表页面 -->
    <div class="tableSample" v-if="!showEdit" key="episodeList">
        <div class="header">
          <div class="header-left">
            <p class="title">句子列表</p>
          </div>
          <div class="header-right">
            <lin-search @query="onQueryChange" placeholder="请输入句子名"/>
...
export default {
  ...
  methods: {
    ...
    handleEdit(self, index, row) {
      ...
      self.editEpisodeId = row.id // eslint-disable-line
    },
    handleDelete(self, index, val) {
      self.$confirm('此操作将永久删除该信息, 是否继续?', '提示', {  // eslint-disable-line
        ...
        const res = await episode.deleteEpisode(val.id).catch(() => {})
