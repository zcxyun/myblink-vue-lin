...
export default {
  ...
  methods: {
    ...
    async _getTableData() {
      ...
      this.loading = true
      try {
        const { total, movies } = await movie.getMovies(page, count, q)
        this._handleData(total, movies)
      } catch (err) {
        this.tableData = []
        console.log(err)
      }
    ...
    async handleCellSave(row, column) {
      ...
      if (prop in row) {
        ...
        const res = await movie.editMovie(data.id, data).catch(() => {
    ...
    handleDelete(self, index, row) {
      self.$confirm('此操作将永久删除该信息, 是否继续?', '提示', {  // eslint-disable-line
        ...
        const res = await movie.deleteMovie(row.id).catch(() => {})
